...
func main() {
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status":"ok","service":"quest"}`))
	})
	...
	r.Post("/api/quest/list", h.ListQuests)
	r.Post("/api/quest/{id}/claim", h.ClaimReward)
	r.Post("/internal/quest/progress", h.ProgressUpdate)
	r.Get("/api/quest/completed", h.CompletedQuests)

	srv := &http.Server{Addr: ":8094", Handler: r}
	go func() {
		slog.Info("quest service starting", "addr", srv.Addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			slog.Error("quest service fatal", "error", err)
			os.Exit(1)
		}
	}()
	...
	slog.Info("quest service shutting down")
