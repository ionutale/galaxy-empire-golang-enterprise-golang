...
type Repository interface {
	CreateResearch(ctx context.Context, playerID int, techType string, targetLevel int, completesAt time.Time) (ResearchQueue, error)
...
func (r *PostgresRepository) CreateResearch(ctx context.Context, playerID int, techType string, targetLevel int, completesAt time.Time) (ResearchQueue, error) {
	var q ResearchQueue
	err := r.pool.QueryRow(ctx,
		`INSERT INTO research.research_queue (player_id, tech_type, target_level, completes_at)
		 VALUES ($1, $2, $3, $4)
		 RETURNING id, player_id, tech_type, target_level, started_at, completes_at, completed, cancelled`,
		playerID, techType, targetLevel, completesAt,
	).Scan(&q.ID, &q.PlayerID, &q.TechType, &q.TargetLevel, &q.StartedAt, &q.CompletesAt, &q.Completed, &q.Cancelled)
	if err != nil {
...
func (r *PostgresRepository) GetActiveResearch(ctx context.Context, playerID int, techType string) (*ResearchQueue, error) {
	var q ResearchQueue
	err := r.pool.QueryRow(ctx,
		`SELECT id, player_id, tech_type, target_level, started_at, completes_at, completed, cancelled
...
func (r *PostgresRepository) ListActiveResearch(ctx context.Context, playerID int) ([]ResearchQueue, error) {
	...
	for rows.Next() {
		var q ResearchQueue
		if err := rows.Scan(&q.ID, &q.PlayerID, &q.TechType, &q.TargetLevel, &q.StartedAt, &q.CompletesAt, &q.Completed, &q.Cancelled); err != nil {
