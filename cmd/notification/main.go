...
func main() {
	...
	internalSecret := getEnv("INTERNAL_SECRET", "internal-secret-change-in-production")
	...
	svc := NewNotificationService(repo, jwtSecret, internalSecret)
	...
	r.Route("/api/notification", func(r chi.Router) {
		r.Get("/list", h.List)
		r.Get("/unread-count", h.UnreadCount)
		r.Post("/{id}/read", h.MarkRead)
		r.Post("/read-all", h.MarkAllRead)
		r.Post("/create", h.CreateInternal)
		r.Get("/stream", h.Stream)
	})

	srv := &http.Server{Addr: ":8093", Handler: r}
	go func() {
		slog.Info("notification service starting", "addr", srv.Addr)
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			slog.Error("notification service fatal", "error", err)
			os.Exit(1)
		}
	}()
