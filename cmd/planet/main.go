...
func runMigrations(ctx context.Context, pool *pgxpool.Pool) error {
	...
	var galaxyCount int
	if err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM galaxy.galaxies`).Scan(&galaxyCount); err != nil {
		return err
	}
	if galaxyCount == 0 {
		if _, err := pool.Exec(ctx, `
	...
	var systemCount int
	if err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM galaxy.systems`).Scan(&systemCount); err != nil {
		return err
	}
	if systemCount == 0 {
		if _, err := pool.Exec(ctx, `
	...
	if err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM galaxy.positions`).Scan(&positionCount); err != nil {
		return err
	}
	if positionCount == 0 {
		if _, err := pool.Exec(ctx, `
